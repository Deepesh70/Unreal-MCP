...
def _render_class_jinja(data: dict, output_dir: str, project_api: str) -> str:
    ...
    search_dirs = [
        ...
        os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "unreal_mcp", "templates"),
    ]
    valid_dirs = [d for d in search_dirs if os.path.isdir(d)]
    if not valid_dirs:
        return f"⚠️  Templates directory not found. Searched: {search_dirs}"

    env = Environment(loader=FileSystemLoader(valid_dirs), keep_trailing_newline=True)
